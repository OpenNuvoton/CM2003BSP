...
volatile uint32_t gu32CapReady;
...
{
    /* Get input Capture status (may also contain CAP0/1/2 pin level bits) */
    ...
    if((gu32Status & ECAP_STATUS_CAPTF0_Msk) == ECAP_STATUS_CAPTF0_Msk)
    {
        ...
        /* Notify main loop only when IC0 capture happened */
        gu32CapReady = 1;
    }
...
void ECAP0_Init(void)
{
    ...
    /* Select IC0 detect rising edge (period between rising edges = input frequency) */
...
void ECAP_FunctionTest()
{
    ...
    /* Configure PB.0 as output mode (signal source), keep PB.1 digital path for ECAP0_IC0 */
    GPIO_SetMode(PB, BIT0, GPIO_MODE_OUTPUT);
    GPIO_ENABLE_DIGITAL_PATH(PB, BIT1);

    /* Clear ECAP interrupt status flags before starting counter */
    gu32Status = ECAP_GET_INT_STATUS(ECAP0);
    ...
    /* Delay 200ms for signal and capture to become stable */
    CLK_SysTickDelay(200000);

    while(1)
    {
        /* Wait for a real IC0 capture interrupt (do not use raw STATUS != 0;
           STATUS also contains CAP0 pin level which can cause a false first hit). */
        if(gu32CapReady != 0)
        {
            uint32_t u32Diff;
            ...
            /* ECAP clock is from PCLK; PCLK0/PCLK1 are HCLK/1 in this sample */
            u32Hz_DET = CLK_GetPCLK1Freq() / (gu32IC0Hold + 1);

            u32Diff = (u32Hz > u32Hz_DET) ? (u32Hz - u32Hz_DET) : (u32Hz_DET - u32Hz);

            /* Accept when frequency is stable (allow small integer rounding jitter) */
            if((u32Hz != 0) && (u32Diff <= 10))
            {
                printf("\nECAP0_IC0 input frequency is %d (Hz), gu32IC0Hold=0x%08X\n", u32Hz_DET, gu32IC0Hold);
                TIMER_Stop(TIMER0);
                break;
            }
