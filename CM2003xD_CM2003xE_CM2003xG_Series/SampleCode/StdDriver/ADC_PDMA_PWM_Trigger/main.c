...
void PDMA_Init()
{
    ...
    /* Open Channel 1 */
    PDMA_Open(PDMA0, BIT1);

    /* transfer width is half word(16 bit) and transfer count is 6 */
    PDMA_SetTransferCnt(PDMA0, 1, PDMA_WIDTH_16, 6);
    ...
    PDMA_SetTransferAddr(PDMA0, 1, (uint32_t)&ADC->ADPDMA, PDMA_SAR_FIX, (uint32_t)g_i32ConversionData, PDMA_DAR_INC);

    /* Select PDMA request source as ADC RX */
    PDMA_SetTransferMode(PDMA0, 1, PDMA_ADC_RX, FALSE, 0);

    /* Set PDMA as single request type for ADC */
    PDMA_SetBurstType(PDMA0, 1, PDMA_REQ_SINGLE, PDMA_BURST_4);
