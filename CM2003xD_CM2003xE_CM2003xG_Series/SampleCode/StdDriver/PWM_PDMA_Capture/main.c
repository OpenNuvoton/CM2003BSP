...
static void GetCaptureExpectedPeriod(PWM_T *pwm, uint32_t u32OutCh, uint32_t u32CapCh,
                                     uint16_t *pu16High, uint16_t *pu16Low, uint16_t *pu16Total)
{
    uint32_t u32OutPs = PWM_GET_PRESCALER(pwm, u32OutCh) + 1;
    uint32_t u32CapPs = PWM_GET_PRESCALER(pwm, u32CapCh) + 1;
    uint32_t u32CMR = PWM_GET_CMR(pwm, u32OutCh);
    uint32_t u32CNR = PWM_GET_CNR(pwm, u32OutCh) + 1;
    uint32_t u32HighOut = u32CMR;
    uint32_t u32LowOut = u32CNR - u32CMR;

    *pu16High = (uint16_t)(u32HighOut * u32OutPs / u32CapPs);
    *pu16Low = (uint16_t)(u32LowOut * u32OutPs / u32CapPs);
    *pu16Total = (uint16_t)(u32CNR * u32OutPs / u32CapPs);
}

/*--------------------------------------------------------------------------------------*/
void CalPeriodTime(PWM_T *PWM, uint32_t u32Ch, uint16_t u16ExpHigh, uint16_t u16ExpLow, uint16_t u16ExpTotal)
{
    ...
    printf("\nPWM generate (capture counter): \nHigh Period=%d, Low Period=%d, Total Period=%d\n",
           u16ExpHigh, u16ExpLow, u16ExpTotal);
    printf("\nCapture Result: Rising Time = %d, Falling Time = %d\n \
               High Period = %d, Low Period = %d, Total Period = %d.\n\n",
           u16RisingTime, u16FallingTime, u16HighPeriod, u16LowPeriod, u16TotalPeriod);
    if((u16HighPeriod < (u16ExpHigh - DEVIATION)) || (u16HighPeriod > (u16ExpHigh + DEVIATION))
            || (u16LowPeriod < (u16ExpLow - DEVIATION)) || (u16LowPeriod > (u16ExpLow + DEVIATION))
            || (u16TotalPeriod < (u16ExpTotal - DEVIATION)) || (u16TotalPeriod > (u16ExpTotal + DEVIATION)))
...
void SYS_Init(void)
{
    ...
    /* Set PA multi-function pins for PWM0 Channel 0 and 2 */
...
int32_t main(void)
{
    ...
    while(1)
    {
        ...
        /* Assume PWM output frequency is 250Hz and duty ratio is 30%, user can calculate PWM settings by follows.
           ...
           cycle time = CNR+1
           High level = CMR+1
           PWM clock source is PCLK (HIRC on CM2003D/E/G).
        ...
        */
        ...
        /* If input minimum frequency is 250Hz, user can calculate capture settings by follows.
           Capture clock source is PCLK. PWM_ConfigCaptureChannel() picks the nearest prescaler.
           CNR is fixed to 0xFFFF in capture mode to increase capture range.
        */
        ...
        {
            uint16_t u16ExpHigh, u16ExpLow, u16ExpTotal;

            GetCaptureExpectedPeriod(PWM0, 0, 2, &u16ExpHigh, &u16ExpLow, &u16ExpTotal);
            CalPeriodTime(PWM0, 2, u16ExpHigh, u16ExpLow, u16ExpTotal);
        }
        /*---------------------------------------------------------------------------------------------------------*/
