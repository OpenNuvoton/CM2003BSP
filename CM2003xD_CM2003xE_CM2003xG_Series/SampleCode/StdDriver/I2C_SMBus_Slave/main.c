...
void SYS_Init(void)
{
    ...
    /* CM2003: I2C MFP default is push-pull — must select open-drain */
    SYS->GPB_MFOS |= (SYS_GPB_MFOS_PB14MFOS_Msk | SYS_GPB_MFOS_PB15MFOS_Msk);
    SYS->GPC_MFOS |= (SYS_GPC_MFOS_PC2MFOS_Msk | SYS_GPC_MFOS_PC3MFOS_Msk);

    /* Weak internal pull-up (external 4.7k to VDD still recommended) */
    PB->PUSEL = (PB->PUSEL & ~((3ul << (14 << 1)) | (3ul << (15 << 1)))) |
                ((GPIO_PUSEL_PULL_UP << (14 << 1)) | (GPIO_PUSEL_PULL_UP << (15 << 1)));
    PC->PUSEL = (PC->PUSEL & ~((3ul << (2 << 1)) | (3ul << (3 << 1)))) |
                ((GPIO_PUSEL_PULL_UP << (2 << 1)) | (GPIO_PUSEL_PULL_UP << (3 << 1)));
...
int32_t main (void)
{
    ...
    while (ch != 0x30)
    {
        ...
        */
        ...
        printf("\n");
        printf("I2C0_SDA(PB.15), I2C0_SCL(PB.14), I2C_ALERT(PC.3), I2C_SUSPEND(PC.2)\n");
