 ...
 * @file     main.c
 * @version  V1.00
 * @brief    Monitor the conversion result of channel 4 by the digital compare function.
 *
...
void SYS_Init(void)
{
    ...
    /* Configure the PB.4 ADC analog input pins.  */
    SYS->GPB_MFPL = (SYS->GPB_MFPL & ~(SYS_GPB_MFPL_PB4MFP_Msk)) |
                    (SYS_GPB_MFPL_PB4MFP_ADC0_CH4);
    /* Disable the PB.4 digital input path to avoid the leakage current. */
...
void ADC_FunctionTest()
{
    ...
    printf("+----------------------------------------------------------------------+\n");
    printf("\nIn this test, software will compare the conversion result of channel 4.\n");

    /* Enable ADC converter */
    ADC_POWER_ON(ADC);

    /* Set input mode as single-end, Single mode, and select channel 4 */
    ADC_Open(ADC, 0, ADC_ADCR_ADMD_SINGLE, BIT4);

    /* Enable ADC comparator 0. Compare condition: conversion result < 0x800; match Count=5 */
    printf("   Set the compare condition of comparator 0: channel 4 is less than 0x800; match count is 5.\n");
    ADC_ENABLE_CMP0(ADC, 4, ADC_ADCMPR_CMPCOND_LESS_THAN, 0x800, 5);

    /* Enable ADC comparator 1. Compare condition: conversion result >= 0x800; match Count=5 */
    printf("   Set the compare condition of comparator 1: channel 4 is greater than or equal to 0x800; match count is 5.\n");
    ADC_ENABLE_CMP1(ADC, 4, ADC_ADCMPR_CMPCOND_GREATER_OR_EQUAL, 0x800, 5);
    ...
    while(1)
    {
        ...
        {
            u32ConversionData = ADC_GET_CONVERSION_DATA(ADC, 4);
            printf("Conversion result of channel 4: 0x%03X (%d)\n", u32ConversionData, u32ConversionData);
    ...
    {
        printf("Comparator 0 interrupt occurs.\nThe conversion result of channel 4 is less than 0x800\n");
    }
    else
    {
        printf("Comparator 1 interrupt occurs.\nThe conversion result of channel 4 is greater than or equal to 0x800\n");
    }
}
