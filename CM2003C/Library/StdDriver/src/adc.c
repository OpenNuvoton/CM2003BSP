 ...
 * @file     adc.c
 * @version  V3.00
 * @brief    CM2003C ADC Driver Source File
 *
 * @note
...
/**
  ...
  * @param[in] u32InputMode Decides the ADC analog input mode. This parameter is unused for CM2003C.
...
void ADC_Open(ADC_T *adc,
              ...
              uint32_t u32ChMask)
{
    adc->ADCR = (adc->ADCR & (~(ADC_ADCR_ADMD_Msk))) | \
                (u32InputMode) | \
                (u32OpMode);
...
/**
  ...
  * @param[in] u32ExtendSampleTime Decides the extend sampling time, the range is from 0 ~ 4095 (0xFFF) ADC clock.
  * @return None
